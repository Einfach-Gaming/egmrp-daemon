...
interface AuthenticatedClient {
  dataBuffer: string
  serverId: number
  serverInfo?: SeverInfo | undefined
  initialized: boolean
  socket: net.Socket
}
...
function parseWhitelist(
  input: string
): ReturnType<typeof whitelistSchema.validateSync> {
  const whitelist = input
    .split(',')
    .filter((addr) => addr.trim().length > 0)
    .map((addr) => {
      const [ip, port] = addr.split(':')
      return { ip: ip?.trim(), port: port ? parseInt(port, 10) : undefined }
    })

  try {
    return whitelistSchema.validateSync(whitelist)
  } catch (err) {
...
const whitelist = parseWhitelist(process.env['WHITELIST'] ?? '')
...
server.on('listening', () => {
  ...
  if (address === null) {
    logger.error('Failed to retrieve TCP server address.')
    process.exit(1)
  }

  logger.info(`Ready at tcp://${address.address}:${address.port}`)
})
